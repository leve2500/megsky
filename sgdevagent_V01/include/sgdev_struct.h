...
#ifndef __SGDEV_STRUCT_H__
#define __SGDEV_STRUCT_H__
...
typedef struct heart_request {
    char            deviceId[DATA_BUF_F64_SIZE];        // 边设备唯一标识
    char            timestamp[DATA_BUF_F32_SIZE];       // 消息发送的时间戳， CST 时间,精度到秒
    char            type[DATA_BUF_F32_SIZE];            // EVENT_HEARTBEAT
}heart_request_s;
...
typedef struct device_upgrade {
    ...
    char            version[DATA_BUF_F64_SIZE];         // 升级后的版本号
...
typedef struct dev_upgrede_res_reply {
    ...
    char                msg[DATA_BUF_F64_SIZE];         // 容器升级结果描述，例如下载地址不可用
...
typedef struct link_dev_info {
    char            name[DATA_BUF_F64_SIZE];               // 接口的名称如为以太网口，则形如“eth1”
    char            status[DATA_BUF_F64_SIZE];             // 设备接口状态，如up/down
}link_dev_info_s;
...
typedef struct dev_sta_reply {
    ...
    char            devDateTime[DATA_BUF_F64_SIZE];         // 设备当前时间
    char            devStDateTime[DATA_BUF_F64_SIZE];       // 设备最近一次启动时间
    uint32_t        devRunTime;                             // 设备运行时长，单位：秒
    link_dev_info_s *linkState;                             // 其他的设备信息，其中每个元素表示一类设备元素
    char            longitude[DATA_BUF_F64_SIZE];           // 地理位置信息经度
    char            latitude[DATA_BUF_F64_SIZE];            // 地理位置信息纬度
    int             link_len;
}dev_sta_reply_s;
...
typedef struct dev_man_conf_command {
    char             devName[DATA_BUF_F64_SIZE];        // 设备名称
...
typedef struct dev_time_command {
    char             dateTime[DATA_BUF_F64_SIZE];       // 日期和时间参数
    char             timeZone[DATA_BUF_F64_SIZE];       // 时区
}dev_time_command_s;
...
typedef struct dev_event_reply {
    char                event[DATA_BUF_F64_SIZE];        // 下表中的一些典型事件用 1001,1002,1003 表示
    char                msg[DATA_BUF_F64_SIZE];          // msg 给出事件具体情况的描述，最长不超过256 字符
}dev_thing_reply_s;

/* 3. (11) 设备日志召回 */
typedef struct dev_log_recall {
    char                url[DATA_BUF_F64_SIZE];         // 文件上传路径
    uint8_t             logType;                        // 日志类型
                                                        //0-全部类型日志； 1-系统日志； 2-操作日志； 3-安全日志； 4-驱动日志；
                                                        //5-broker日志、 6-审计日志； 7-调试日志； 8-255 用于扩展。
}dev_log_recall_s;
...
typedef struct container_install_cmd {
    ...
    char                container[DATA_BUF_F64_SIZE];   // 容器名称
    file_info_s         image;                          // 容器镜像
    with_app_info_s     withAPP;                        // 随容器安装时，被添加的 APP 名称
    cfg_cpu_info_s      cfgCpu;                         // cpu 资源配置参数
    cfg_mem_info_s      cfgMem;                         // memory 资源配置参数
    cfg_disk_info_s     cfgDisk;                        // disk 资源配置参数
    char                port[DATA_BUF_F64_SIZE];        // 容器端口资源配置参数
    char                mount[DATA_BUF_F64_SIZE][DATA_BUF_F64_SIZE]; // 映射的本地文件目录资源配置参数
    char                dev[DATA_BUF_F64_SIZE][DATA_BUF_F64_SIZE];   // 映射的本地物理接口资源配置参数
    uint16_t            mount_len;
    uint16_t            dev_len;
...
typedef struct container_conf_cmd {
    ...
    char                port[DATA_BUF_F64_SIZE];       // 端口资源配置参数
    char                mount[DATA_BUF_F64_SIZE][DATA_BUF_F64_SIZE]; // 映射的本地文件目录资源配置参数
    char                dev[DATA_BUF_F64_SIZE][DATA_BUF_F64_SIZE];   // 映射的本地物理接口资源配置参数
    uint16_t            mount_len;
    uint16_t            dev_len;
...
typedef struct container_status_reply {
    char                container[DATA_BUF_F64_SIZE];   // 容器名称
    char                version[DATA_BUF_F64_SIZE];     // 容器版本号
    char                state[DATA_BUF_F64_SIZE];       // 容器运行状态， running 或 stopped
    int                 cpuRate;                        // CPU 占用率，百分比
    int                 memUsed;                        // 内存占用率，百分比
    int                 diskUsed;                       // 磁盘占用率，百分比
    char                ip[DATA_BUF_F64_SIZE];          // IP 地址及端口
    char                created[DATA_BUF_F64_SIZE];     // 创建时间
    char                started[DATA_BUF_F64_SIZE];     // 最近一次启动时间
    time_t              lifeTime;                       // 运行时间，单位：秒
    char                image[DATA_BUF_F64_SIZE];       // 容器镜像信息
    int                 container_len;
...
typedef struct container_event_report {
    char                container[DATA_BUF_F64_SIZE];  // 容器名称
    char                event[DATA_BUF_F64_SIZE];      // 下表中的一些典型事件用 4001,4002,4003 表示
    char                msg[DATA_BUF_F64_SIZE];        // msg 给出事件具体情况的描述，最长不超过 256字符
...
typedef struct container_log_recall_cmd {
    char                container[DATA_BUF_F64_SIZE];  // 容器名称，如果容器名称不出现，则召回所有容器的日志
    char                url[DATA_BUF_F64_SIZE];        // 文件上传路径
...
typedef struct app_install_cmd {
    ...
    char                version[DATA_BUF_F64_SIZE];     // 应用版本号
    file_info_s         file;                           // 升级的 APP，详见附录 A
    cfg_cpu_info_s      cfgCpu;                         // cpu 资源配置参数,详见附录 A
    cfg_mem_info_s      cfgMem;                         // memory 资源配置参数,详见附录 A
    char                enable[DATA_BUF_F64_SIZE];      // 使能/去使能状态定义，使能为“1”，去使能为“0”
    char                app[DATA_BUF_F64_SIZE];         // 应用名称
}app_install_cmd_s;
...
typedef struct app_conf_cmd {
    char                container[DATA_BUF_F64_SIZE];   // 容器名字
    char                app[DATA_BUF_F64_SIZE];         // 应用文件名字
    cfg_cpu_info_s      cfgCpu;                         // cpu 资源配置参数,详见附录 A
    cfg_mem_info_s      cfgMem;                         // memory 资源配置参数,详见附录 A
}app_conf_cmd_s;
...
typedef struct process_info {
    ...
    char                srvName[DATA_BUF_F64_SIZE];     // 进程名称
    char                srvEnable[DATA_BUF_F64_SIZE];   // 服务使能状态， yes 或 no
    char                srvStatus[DATA_BUF_F64_SIZE];   // 服务状态， running 或 stopped
    ...
    char                startTime[DATA_BUF_F64_SIZE];   // 表示服务启动时间，百分比数据
}process_info_s;

// 表74 Array<apps>报文字段说明
typedef struct apps_info {
    char                app[DATA_BUF_F64_SIZE];         // APP 名称
    char                version[DATA_BUF_F32_SIZE];     // APP 版本
    char                appHash[DATA_BUF_F64_SIZE];     // APP 的哈希值
...
typedef struct app_status_reply {
    char                container[DATA_BUF_F64_SIZE];        // 容器名称
    apps_info_s         apps[DATA_BUF_F16_SIZE];             // app 状态参数
    uint16_t            apps_num;
}app_status_reply_s;
...
typedef struct app_event_reply {
    char                container[DATA_BUF_F64_SIZE];        // 容器名称
    char                app[DATA_BUF_F64_SIZE];              // app 名称，如果 APP 名称不出现，则上报的是所有 APP 的统一事件
    char                event[DATA_BUF_F64_SIZE];            // 见表 80
    char                msg[DATA_BUF_F64_SIZE];              // msg 给出事件具体情况的描述，最长不超过256 字符
}app_event_reply_s;
...
typedef struct app_upgrade_cmd {
    int32_t         jobId;                              // 本升级操作作为一个工作任务，分配的 ID
    uint32_t        policy;                             // 从接收到该升级指令时间后开始升级的时间间隔（单位：秒
                                                        // 缺省或等于 0 时，表示立即升级
    char            version[DATA_BUF_F64_SIZE];         // 升级后的版本号
    char            container[DATA_BUF_F64_SIZE];       // 容器名称
...
typedef struct app_log_recall_cmd {
    char              container[DATA_BUF_F64_SIZE];  // 容器名称
    char              url[DATA_BUF_F64_SIZE];        // 文件上传路径
    char              app[DATA_BUF_F64_SIZE];        // app 名称，如果 APP 名称不出现，则召回的是所有 APP 的日志
